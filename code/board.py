...
class Board:
    ...
    def calc_moves(self,piece,row,col,bool=True):
        '''
        calculate all the possible valid move of a specific move in a specific position 
        '''
            ...
            if not piece.moved:
                ...
                if isinstance(left_rook,Rook):
                    if not left_rook.moved:
                        for c in range(1,4):
                            if self.squares[row][c].has_piece(): # castling not possible due to piece in between
                                break
